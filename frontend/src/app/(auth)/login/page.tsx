...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Prefetch common dashboard routes so they load instantly after login
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem("edunexus_user", JSON.stringify(data.user));
      
      // Set cookie on client-side so middleware can intercept it on page navigation
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-gray-200 flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-md bg-white/[0.02] border border-white/[0.05] p-8 rounded-2xl shadow-2xl backdrop-blur-xl relative z-10">
        <h2 className="text-3xl font-bold mb-2 bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent">EduNexus Portal</h2>
