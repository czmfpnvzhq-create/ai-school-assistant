"use client";

import React, { useState, useEffect, useCallback } from "react";
import { useRouter } from "next/navigation";
import { API_BASE_URL } from "@/lib/config";
import { useTeacherData, getToken } from "@/lib/useTeacherData";

interface StudentGradeRow {
  studentId: number;
  studentName: string;
  score: number | null;
}

const SUBJECT_OPTIONS = ["Math", "Science", "English", "Urdu", "Islamiat"];

function SkeletonRows() {
  return (
    <>
      {[1, 2, 3, 4, 5].map((i) => (
        <tr key={i}>
          <td className="px-6 py-4">
            <div className="h-4 w-36 bg-slate-800 rounded animate-pulse" />
          </td>
          <td className="px-6 py-4 text-right">
            <div className="ml-auto h-10 w-32 bg-slate-800 rounded-xl animate-pulse" />
          </td>
        </tr>
      ))}
    </>
  );
}

export default function TeacherGradesPage() {
  const router = useRouter();
  const { info, isLoading: isLoadingClass } = useTeacherData();

  // Filter params state
  const [selectedSubject, setSelectedSubject] = useState<string>("Math");
  const [examDate, setExamDate] = useState<string>(
    new Date().toISOString().split("T")[0]
  );

  // Students list and active input scores state
  const [students, setStudents] = useState<StudentGradeRow[]>([]);
  const [scoresState, setScoresState] = useState<Record<number, string>>({});

  // Loading & status states
  const [isLoadingStudents, setIsLoadingStudents] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [isLocked, setIsLocked] = useState(false);
  const [errorMessage, setErrorMessage] = useState<string | null>(null);
  const [successMessage, setSuccessMessage] = useState<string | null>(null);

  // Fetch students + existing grades
  const fetchStudentGrades = useCallback(async () => {
    if (!info?.classId || !selectedSubject || !examDate) return;

    setIsLoadingStudents(true);
    setErrorMessage(null);
    setSuccessMessage(null);

    try {
      const token = getToken();
      if (!token) { router.push("/login"); return; }

      const queryParams = new URLSearchParams({
        classId: info.classId.toString(),
        subject: selectedSubject,
        examDate: examDate,
      });

      const res = await fetch(`${API_BASE_URL}/grades?${queryParams.toString()}`, {
        headers: { Authorization: `Bearer ${token}` },
      });

      if (res.status === 401) {
        localStorage.removeItem("edunexus_token");
        router.push("/login");
        return;
      }

      if (!res.ok) throw new Error("Failed to load student grades.");

      const data: StudentGradeRow[] = await res.json();
      setStudents(data);

      // Prepopulate scores inputs
      const initialScores: Record<number, string> = {};
      data.forEach((s) => {
        initialScores[s.studentId] = s.score !== null ? s.score.toString() : "";
      });
      setScoresState(initialScores);

      const hasSavedGrades = data.length > 0 && data.some((s) => s.score !== null);
      setIsLocked(hasSavedGrades);
    } catch (err: any) {
      setErrorMessage(err.message || "Failed to fetch student grades.");
    } finally {
      setIsLoadingStudents(false);
    }
  }, [info?.classId, selectedSubject, examDate, router]);

  useEffect(() => {
    fetchStudentGrades();
  }, [fetchStudentGrades]);

  const handleScoreChange = (studentId: number, valStr: string) => {
    setScoresState((prev) => ({
      ...prev,
      [studentId]: valStr,
    }));
  };

  const handleSaveGrades = async () => {
    setIsSaving(true);
    setErrorMessage(null);
    setSuccessMessage(null);

    try {
      const token = getToken();
      if (!token) { router.push("/login"); return; }

      const gradesArray = students.map((s) => {
        const val = scoresState[s.studentId];
        if (val === undefined || val.trim() === "") {
          throw new Error(`Please enter a score for student: ${s.studentName}`);
        }

        const scoreNum = parseFloat(val);
        if (isNaN(scoreNum) || scoreNum < 0 || scoreNum > 100) {
          throw new Error(
            `Invalid score for ${s.studentName}. Score must be a number between 0 and 100.`
          );
        }

        return {
          studentId: s.studentId,
          subject: selectedSubject,
          score: scoreNum,
          examDate: examDate,
        };
      });

      const res = await fetch(`${API_BASE_URL}/grades`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
        body: JSON.stringify({ grades: gradesArray }),
      });

      if (!res.ok) throw new Error("Failed to save student grades.");

      setSuccessMessage("All grades saved successfully!");
      setTimeout(() => setSuccessMessage(null), 3000);
      fetchStudentGrades();
    } catch (err: any) {
      setErrorMessage(err.message || "An unexpected error occurred.");
    } finally {
      setIsSaving(false);
    }
  };

  const showSkeleton = isLoadingClass || (isLoadingStudents && students.length === 0);

  return (
    <div className="space-y-6 pb-8 animate-fade-in-up">
      {/* Header */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">My Class Grades</h1>
          <p className="text-slate-400 text-sm mt-1">Record and edit exam scores for {info?.className || "your class"}.</p>
        </div>
      </div>

      {/* Filters Panel */}
      <div className="bg-slate-900/50 backdrop-blur-md rounded-2xl border border-white/5 p-5 shadow-xl grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label className="block text-xs font-bold text-slate-400 uppercase tracking-wide mb-2">Class</label>
          {isLoadingClass ? (
            <div className="h-11 w-full bg-slate-800 animate-pulse rounded-xl" />
          ) : (
            <div className="w-full h-11 bg-slate-950/50 border border-white/5 rounded-xl px-4 flex items-center text-sm text-slate-200">
              {info?.className || "No class assigned"}
            </div>
          )}
        </div>

        <div>
          <label className="block text-xs font-bold text-slate-400 uppercase tracking-wide mb-2">Subject</label>
          <select
            value={selectedSubject}
            onChange={(e) => setSelectedSubject(e.target.value)}
            className="w-full h-11 bg-slate-950/50 border border-white/5 focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 rounded-xl px-4 text-sm text-slate-200 outline-none transition-all cursor-pointer"
          >
            {SUBJECT_OPTIONS.map((sub) => (
              <option key={sub} value={sub}>{sub}</option>
            ))}
          </select>
        </div>

        <div>
          <label className="block text-xs font-bold text-slate-400 uppercase tracking-wide mb-2">Exam Date</label>
          <input
            type="date"
            value={examDate}
            onChange={(e) => setExamDate(e.target.value)}
            className="w-full h-11 bg-slate-950/50 border border-white/5 focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 rounded-xl px-4 text-sm text-slate-200 outline-none transition-all"
          />
        </div>
      </div>

      {/* Messages */}
      {errorMessage && (
        <div className="bg-rose-950/40 border border-rose-900/50 text-rose-400 px-5 py-4 rounded-xl text-sm shadow-lg flex items-center justify-between">
          <span>⚠️ {errorMessage}</span>
          <button onClick={() => setErrorMessage(null)} className="text-rose-400 hover:text-rose-200">✕</button>
        </div>
      )}
      {successMessage && (
        <div className="bg-emerald-950/40 border border-emerald-900/50 text-emerald-400 px-5 py-4 rounded-xl text-sm shadow-lg flex items-center justify-between">
          <span>✨ {successMessage}</span>
          <button onClick={() => setSuccessMessage(null)} className="text-emerald-400 hover:text-emerald-200">✕</button>
        </div>
      )}

      {/* Lock Banner */}
      {isLocked && !showSkeleton && (
        <div className="bg-blue-950/40 border border-blue-900/50 text-blue-300 px-5 py-4 rounded-xl text-sm shadow-lg flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 animate-fade-in">
          <span className="flex items-center gap-2">🔒 Grades are saved and locked for this exam date and subject.</span>
          <button
            onClick={() => setIsLocked(false)}
            className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-1.5 px-4 rounded-lg text-xs transition-all shrink-0 self-start sm:self-auto"
          >
            🔓 Modify Grades
          </button>
        </div>
      )}

      {/* Student List & Inputs */}
      <div className="bg-slate-900/50 backdrop-blur-md rounded-2xl border border-white/5 shadow-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm text-slate-400">
            <thead className="text-xs text-slate-500 uppercase bg-slate-950/50">
              <tr>
                <th className="px-6 py-4 rounded-l-lg">Student Name</th>
                <th className="px-6 py-4 text-right rounded-r-lg">Score (0-100)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {showSkeleton ? (
                <SkeletonRows />
              ) : !info?.classId ? (
                <tr>
                  <td colSpan={2} className="px-6 py-16 text-center text-slate-500">
                    You do not have a class assigned.
                  </td>
                </tr>
              ) : students.length === 0 ? (
                <tr>
                  <td colSpan={2} className="px-6 py-16 text-center text-slate-500">
                    No students found in this class.
                  </td>
                </tr>
              ) : (
                students.map((student) => {
                  const val = scoresState[student.studentId] ?? "";
                  return (
                    <tr key={student.studentId} className="hover:bg-white/[0.01] transition-colors animate-fade-in">
                      <td className="px-6 py-4 font-semibold text-slate-200">{student.studentName}</td>
                      <td className="px-6 py-4 text-right">
                        <input
                          type="number"
                          min="0"
                          max="100"
                          value={val}
                          disabled={isLocked}
                          placeholder="e.g. 85"
                          onChange={(e) => handleScoreChange(student.studentId, e.target.value)}
                          className="w-32 h-10 bg-slate-950/50 border border-white/5 focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 rounded-xl px-4 text-sm text-slate-200 outline-none text-right transition-all font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
                        />
                      </td>
                    </tr>
                  );
                })
              )}
            </tbody>
          </table>
        </div>
      </div>

      {/* Save Button */}
      {students.length > 0 && !showSkeleton && !isLocked && (
        <div className="flex justify-end mt-4">
          <button
            onClick={handleSaveGrades}
            disabled={isSaving}
            className="w-full md:w-auto min-w-[200px] bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 disabled:opacity-50 text-white font-bold py-3 px-8 rounded-xl transition-all duration-300 shadow-lg transform hover:-translate-y-0.5 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            {isSaving ? (
              <>
                <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                Saving Grades...
              </>
            ) : (
              <>💾 Save All Grades</>
            )}
          </button>
        </div>
      )}
    </div>
  );
}
