...
export default function GradesPage() {
  ...
  const { classes, isLoading: isLoadingClasses } = useAdminClasses();
  ...
  const fetchStudentGrades = useCallback(async () => {
    if (!selectedClassId || !selectedSubject || !examDate) return;
  ...
  }, [selectedClassId, selectedSubject, examDate, router]);

  useEffect(() => {
    fetchStudentGrades();
  }, [fetchStudentGrades]);

  // Update input state value with validation checks
  ...
  const handleSaveGrades = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error("Failed to save student grades.");
      }
  ...
  return (
    <div className="space-y-6 pb-8 animate-fade-in-up">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-white tracking-tight">Grades Management</h1>
          <p className="text-slate-400 text-sm mt-1">Record and edit exam scores for subjects per class.</p>
        </div>
        <button
          onClick={() => router.push("/dashboard/admin/grades/report")}
